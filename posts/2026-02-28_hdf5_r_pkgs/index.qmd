...
* **Smart Partial I/O (Upcoming):** One of HDF5's greatest strengths is reading small slices of massive datasets. Support for ["smart" partial I/O](https://cmmr.github.io/h5lite/articles/partial-reading.html) is being introduced in **version 2.0.0.3**. This version allows you to use `start` and `count` parameters to target specific structural units without loading entire files into memory.
...
* **Core Compression:** To maintain a minimal footprint, `hdf5lib` bundles only **zlib**. This ensures that standard GZIP compression "just works" out-of-the-box on all platforms without adding unnecessary build time for advanced filters.

* **Thread-Safety:** Compiled with thread-safety enabled by default, allowing for [safe parallel I/O](https://cmmr.github.io/hdf5lib/articles/parallelism.html) regardless of your chosen threading system.
...
## Modular Compression: The [`h5filters`](https://github.com/cmmr/h5filters/) Package
    
To keep the core packages lean while providing world-class compression, we are introducing a modular "Plugin" approach. 

* **The Upcoming `h5filters` Package:** We are currently developing a companion package, `h5filters`. This package will act as a "one-stop shop" for high-performance HDF5 filters, including **LZ4**, **Zstd**, **Blosc**, and **Szip (libaec)**.

* **How it Works:** Once installed, `h5lite` will automatically detect `h5filters` and register these specialized compressors at runtime.

* **Immediate Alternatives:** If you need these formats today, `h5lite` can utilize external plugins. You can point the `HDF5_PLUGIN_PATH` environment variable to the plugin directories provided by Bioconductor's **`rhdf5filters`**, Python’s **`hdf5plugin`** package, or Julia's **`H5Z`** family of packages.
